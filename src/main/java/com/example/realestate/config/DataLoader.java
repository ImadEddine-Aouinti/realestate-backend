...
public class DataLoader implements CommandLineRunner {
    ...
    private void createSampleUsers() {
        if (userRepository.count() == 0) {
            ...
            System.out.println("✅ " + users.size() + " utilisateurs de démonstration créés avec succès");
        }
    }
    ...
    private void createSampleProperties() {
        if (propertyRepository.count() == 0) {
            ...
            User adminOwner = users.get(0);
            User client1Owner = users.get(1);
            User client2Owner = users.get(2);
            User client3Owner = users.get(3);
            User client4Owner = users.get(4);
            User client5Owner = users.get(5);

            // Créer une factory pour les points géographiques avec SRID 4326
            GeometryFactory geometryFactory = new GeometryFactory(new PrecisionModel(), 4326);

            // Coordonnées pour Paris
            Point paris1Location = geometryFactory.createPoint(new Coordinate(2.3076, 48.8706));
            Point paris2Location = geometryFactory.createPoint(new Coordinate(2.3422, 48.8566));

            // Coordonnées pour Lyon
            Point lyonLocation = geometryFactory.createPoint(new Coordinate(4.8357, 45.7640));

            // Coordonnées pour Marseille
            Point marseilleLocation = geometryFactory.createPoint(new Coordinate(5.3698, 43.2965));

            // Coordonnées pour Toulouse
            Point toulouseLocation = geometryFactory.createPoint(new Coordinate(1.4442, 43.6045));

            List<Property> properties = Arrays.asList(
                    createPropertyWithLocation(client1Owner, "Villa Moderne avec Piscine",
                    ...
                    createPropertyWithLocation(client2Owner, "Appartement Centre-Ville Lumineux",
                    ...
                    createPropertyWithLocation(client3Owner, "Maison de Famille Spacieuse",
                    ...
                    createPropertyWithLocation(client4Owner, "Duplex avec Terrasse Panoramique",
                    ...
                    createPropertyWithLocation(client5Owner, "Studio Étudiant Proche Université",
            ...
            System.out.println("✅ " + properties.size() + " propriétés de démonstration créées avec succès, chacune avec ses images et coordonnées géographiques");
        }
    }

    private Property createPropertyWithLocation(User owner, String title, String description, BigDecimal price,
